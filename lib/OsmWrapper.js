...
module.exports = class OsmWrapper {
  ...
  constructor(lat, lon, latCoverage, lonCoverage, settings) {
    ...
    this.listeners = !!settings ? settings.listeners || {} : {};
    this.elements;
  ...
  async updateMapData(attempts = 3) {
    ...
    try {
      ...
      await this.parser.action({
        ...
        this.downloadingMapData = false;
        this.currentDataCenter = this.getCurrentDataCenter();
        if (!!this.listeners.onMapUpdate) {
          this.listeners.onMapUpdate();
        }
  ...
  getCollisions(player) {
    ...
    if (!player) {
      player = {
        x: this.viewport.w * 0.5,
        y: this.viewport.h * 0.5
      };
    }
  ...
  minmax() {
    ...
    const result = {minX: false, maxX: false, minY: false, maxY: false, minLat: false, maxLat: false, minLon: false, maxLon: false, totalVertices: 0, outOfViewport: 0};
    let totalVertices = 0,
        outOfViewport = 0;
    for (let el of this.elements) {
      for (let [x, y] of el.points) {
        result.totalVertices++;
        if (x < 0 || x >= this.viewport.w || y < 0 || y >= this.viewport.h) {
          result.outOfViewport++;
        }
  ...
  log() {
    ...
    const logTemplate = {
      ...
      "Landmarks": new Set(),
      "Streets": new Set(),
      "Coverage (km^2)": OsmElement.measureArea(this.minmax()),
  ...
  static async peek(lat, lon) {

    const osm = new OsmWrapper(lat, lon, 0.001, null);
    await osm.init();
    return osm.getCollisions();
